module CMA
  class Adjuster
    LAST_VALUE_WINS =
      ['Account ID', 'Account Name', 'Campaign', 'Ad Group',
    ...
    LAST_REAL_VALUE_WINS =
      ['Last Avg CPC', 'Last Avg Pos']

    INT_VALUES =
      ['Clicks', 'Impressions', 'ACCOUNT - Clicks',
       'CAMPAIGN - Clicks', 'BRAND - Clicks',
       'BRAND+CATEGORY - Clicks',
       'ADGROUP - Clicks', 'KEYWORD - Clicks']

    FLOAT_VALUES =
      ['Avg CPC', 'CTR', 'Est EPC', 'newBid', 'Costs', 'Avg Pos']

    ADJUST_BY_CANCELLATION =
      ['number of commissions']

    ADJUST_BY_CANCELLATION_AND_SALESAMOUNT =
      ['Commission Value',
      ...
      def adjust_values(hash)
        ...
        ADJUST_BY_CANCELLATION_AND_SALESAMOUNT.each do |key|
          hash[key] = (@cancellation_factor * @saleamount_factor *
                       hash.fetch(key, '0').from_german_to_f).to_german_s
        end
        hash
      end
  end
